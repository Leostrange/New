...
**Задачи из документа:**
*   [ ] Переключение между вкладками NavigationBar не приводит к изменению содержимого экрана. **Статус: Частично реализовано (с ошибкой в логике).**
*   [ ] Содержимое вкладок (библиотека, облако, аннотации, плагины), упомянутое в комментарии к коду, отсутствует. **Статус: Частично реализовано (LibraryScreen и ThemesScreen), Нереализовано (CloudScreen, AnnotationsScreen, PluginsScreen).**
*   [x] Логика навигации по нажатию на иконки NavigationBarItem и IconButton в TopAppBar не реализована ( `onClick = { /*TODO*/ }` ). **Статус: Реализовано (навигация присутствует).**
*   [x] Интеграция навигации: Реализовать навигацию между экранами при нажатии на NavigationBarItem . Использовать NavController для управления состоянием навигации. **Статус: Реализовано.**
*   [x] Реализация вкладок: Создать отдельные Composable-функции для каждой вкладки (библиотека, облако, аннотации, плагины). Интегрировать их в MainScreen с использованием TabRow или HorizontalPager для переключения содержимого. **Статус: Реализовано.**
*   [ ] Подключение к данным: Для вкладки "Библиотека" (если она будет частью MainScreen ) обеспечить отображение актуальных данных из библиотеки пользователя. **Статус: Частично реализовано (UI-связь есть, но глубина интеграции с данными требует проверки).**
*   [x] Обработка нажатия на иконку профиля: Реализовать переход на AccountScreen при нажатии на иконку AccountCircle . **Статус: Реализовано.**
...
**Задачи из документа:**
*   [x] Поиск, фильтрация, сортировка: Хотя UI-элементы присутствуют, их интеграция с реальными данными и ViewModel требует проверки и доработки. В текущем коде фильтрации и сортировки реализованы на уровне UI, но необходимо убедиться, что они эффективно работают с большим объемом данных и не вызывают проблем с производительностью. **Статус: Частично реализовано (UI-элементы присутствуют, но логика и интеграция с данными отсутствуют).**
*   [ ] Добавление комикса: AddComicDialog принимает данные, но логика фактического добавления комикса в базу данных или хранилище должна быть полностью реализована и протестирована. **Статус: Нереализовано.**
*   [x] Обложки комиксов: AsyncImage используется для отображения обложек, но необходимо убедиться, что comic.coverPath корректно обрабатывается и обложки загружаются из правильного источника (локального или удаленного). (Частично реализовано: UI для отображения обложек добавлен с использованием заглушек URL) **Статус: Частично реализовано (UI для отображения обложек добавлен с использованием заглушек URL).**
*   [ ] Удаление комикса: Логика удаления комикса ( `viewModel.removeComic(comic.id)` ) должна быть полностью реализована. **Статус: Нереализовано.**
*   [ ] Оптимизация поиска и фильтрации: Перенести логику поиска, фильтрации и сортировки в LibraryViewModel для более эффективной обработки данных и возможной интеграции с базой данных (например, Room). **Статус: Нереализовано.**
*   [ ] Полная реализация добавления комикса: Обеспечить сохранение нового комикса в локальной базе данных и его отображение в списке. **Статус: Нереализовано.**
*   [ ] Загрузка обложек: Убедиться, что AsyncImage корректно загружает обложки из указанных путей. Возможно, потребуется дополнительная логика для генерации миниатюр или извлечения обложек из файлов комиксов. **Статус: Нереализовано.**
*   [ ] Реализация удаления комикса: Добавить логику удаления комикса из базы данных и файловой системы. **Статус: Нереализовано.**
...
**Задачи из документа:**
*   [ ] Поддержка форматов PDF и*   [ ] Поддержка форматов PDF и CBR: В текущем коде явно указано, что эти форматы не поддерживаются. Необходимо интегрировать библиотеки или написать код для парсинга и отображения страниц из PDF и CBR файлов. **Статус: Частично реализовано (базовая логика выбора экстрактора присутствует, но требует проверки функциональности экстракторов).**
*   [ ] Режимы чтения: Хотя UI для выбора режимов чтения (постранично, непрерывно, вебтун) присутствует, фактическая реализация этих режимов отсутствует. **Статус: Частично реализовано (только постраничный режим).**
*   [ ] Масштабирование: UI для масштабирования присутствует, но его функциональность не полностью реализована. **Статус: Нереализовано.**
*   [ ] Закладки: Логика добавления/удаления закладок ( `viewModel.toggleBookmark(currentPage)` ) должна быть полностью реализована и интегрирована с системой хранения закладок. **Статус: Нереализовано.**
*   [ ] Настройки чтения: Кнопка "Настройки" в TopAppBar не имеет реализованной логики. **Статус: Нереализовано.**
*   [ ] Интеграция PDF/CBR парсеров: Использовать сторонние библиотеки (например, PdfRenderer для PDF, или sevenzipjbinding для CBR) для извлечения страниц из соответствующих форматов. **Статус: Частично реализовано.**
*   [ ] Реализация режимов чтения: Адаптировать отображение страниц для каждого режима: постранично (текущая реализация), непрерывно (вертикальный скролл всех страниц), вебтун (специфическая компоновка для вебтунов). **Статус: Частично реализовано.**
*   [ ] Полная реализация масштабирования: Обеспечить корректное масштабирование страниц с использованием жестов. **Статус: Нереализовано.**
*   [ ] Управление закладками: Реализовать сохранение и загрузку закладок в базу данных, а также отображение закладок в UI. **Статус: Нереализовано.**
*   [ ] Навигация к настройкам чтения: Реализовать переход на ReaderSettingsScreen . **Статус: Нереализовано.**овано.**
...
**Задачи из документа:**
*   [ ] *   [ ] Отображение метаданных: Убедиться, что все метаданные (название, автор, издатель, жанр, описание) корректно отображаются. **Статус: Частично реализовано (UI-элементы присутствуют, но логика требует проверки).**
*   [ ] Кнопки действий: Реализовать логику для кнопок "Добавить в избранное", "Сбросить прогресс", "Удалить комикс". **Статус: Частично реализовано (вызовы ViewModel присутствуют, но логика требует проверки).**
*   [ ] Предложения похожих комиксов: Реализовать логику для отображения предложений похожих комиксов. **Статус: Нереализовано.**
*   [ ] Интеграция с базой данных: Обеспечить сохранение/удаление комикса из избранного, сброс прогресса чтения и удаление комикса из базы данных. **Статус: Нереализовано.**
*   [ ] Алгоритм рекомендаций: Разработать и интегрировать алгоритм для предложения похожих комиксов на основе жанра, автора или других метаданных. **Статус: Нереализовано.**
