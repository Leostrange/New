...
public final class hw
{
    ...
    public static PackageInfo a(Context context, Intent intent)
    {
        ...
        list = context.queryIntentActivities(intent, 0);
...
_L2:
        if ((intent = context.resolveActivity(intent, 0)) != null)
        {
            ...
            try
            {
                context = context.getPackageInfo(((ResolveInfo) (intent)).activityInfo.packageName, 64);
            }
            ...
            intent = ((PackageInfo) (context)).signatures;
            k = intent.length;
            i = 0;
            while (i < k) 
            {
