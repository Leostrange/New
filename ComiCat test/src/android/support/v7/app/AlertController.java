...
_L9:
    ...
    public AlertController(Context context, AppCompatDialog appcompatdialog, Window window)
    {
        ...
        context = context.obtainStyledAttributes(null, cv.k.AlertDialog, cv.a.alertDialogStyle, 0);
        mAlertDialogLayout = context.getResourceId(cv.k.AlertDialog_android_layout, 0);
        mButtonPanelSideLayout = context.getResourceId(cv.k.AlertDialog_buttonPanelSideLayout, 0);
        mListLayout = context.getResourceId(cv.k.AlertDialog_listLayout, 0);
        mMultiChoiceItemLayout = context.getResourceId(cv.k.AlertDialog_multiChoiceItemLayout, 0);
        mSingleChoiceItemLayout = context.getResourceId(cv.k.AlertDialog_singleChoiceItemLayout, 0);
        mListItemLayout = context.getResourceId(cv.k.AlertDialog_listItemLayout, 0);
        context.recycle();
    }
    ...
    {
        mScrollView = (ScrollView)mWindow.findViewById(cv.f.scrollView);
    ...
    private boolean setupTitle(ViewGroup viewgroup)
    {
        if (mCustomTitleView != null)
        {
            ...
            mWindow.findViewById(cv.f.title_template).setVisibility(8);
            return true;
        }
        ...
        if (flag)
        {
            mTitleView = (TextView)mWindow.findViewById(cv.f.alertTitle);
    ...
    {
        boolean flag = false;
        setupContent((ViewGroup)mWindow.findViewById(cv.f.contentPanel));
        boolean flag1 = setupButtons();
        Object obj = (ViewGroup)mWindow.findViewById(cv.f.topPanel);
        es es1 = es.a(mContext, null, cv.k.AlertDialog, cv.a.alertDialogStyle);
        setupTitle(((ViewGroup) (obj)));
        obj = mWindow.findViewById(cv.f.buttonPanel);
        if (!flag1)
        {
            ((View) (obj)).setVisibility(8);
            obj = mWindow.findViewById(cv.f.textSpacerNoButtons);
            if (obj != null)
            {
                ((View) (obj)).setVisibility(0);
            }
        }
        FrameLayout framelayout = (FrameLayout)mWindow.findViewById(cv.f.customPanel);
        if (mView != null)
        {
            obj = mView;
        } else
        ...
        if (flag)
        {
            FrameLayout framelayout1 = (FrameLayout)mWindow.findViewById(cv.f.custom);
