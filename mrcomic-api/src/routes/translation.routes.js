...
const { spawn } = require("child_process");
    ...
    const pythonProcess = spawn("python3", ["../../pipeline/integrated_system.py", "translate", text, targetLanguage]);
    ...
    pythonProcess.on("close", (code) => {
      if (code !== 0) {
        ...
        return res.status(500).json({
          code: "python_error",
          message: "Ошибка при выполнении Python скрипта перевода",
        });
      }
      try {
    ...
    res.status(500).json({
      code: "server_error",
      message: "Ошибка сервера при переводе текста",
    });
  }
});
